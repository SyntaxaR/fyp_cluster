...
async def register_worker(heartbeat: WorkerHeartbeat, worker_id: int=-1) -> bool:
    ...
    ws_status = await workers_ws_manager.connect_to_worker(worker_id, registration.control_ip)
    if ws_status:
    ...
    while True:
        ...
        for worker_id, registration in registered_workers.items():
            if registration.timestamp < threshold_time:
                ...
                try:
                    await workers_ws_manager.connect_to_worker(worker_id, registration.control_ip)
                except Exception as e:
